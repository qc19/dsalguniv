...
int getMaxProfitUtil(int ind, bool buy, int n, int stockPrice[n])
{
    ...
    if (buy){
        ...
        profit = MAX((getMaxProfitUtil(ind+2, true, n, stockPrice) + (stockPrice[ind])),
...
int getMaxProfitUtilMemoization(int ind, bool buy, int n, int stockPrice[n], int dp[][2])
{
    ...
    if (buy){
        ...
        dp[ind][buy] = MAX((getMaxProfitUtilMemoization(ind+2, true, n, stockPrice, dp) + (stockPrice[ind])),
    ...
    for (int ind = n-1; ind >= 0; ind--){
        for (int buy = 0; buy <= 1; buy++){
            if (buy){
                ...
                dp[ind][buy] = MAX((dp[ind+2][1] + stockPrice[ind]), (dp[ind+1][0] + 0));
            }
        }
    }
...
// Cooldown condition, Cannot Buy right next day after selling previous day
