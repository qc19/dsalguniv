...
/*
    ...
    already included in MST and other contains rest of the vertices), pick the minimum weight edge
    ...
    the key value for these vertices indicate the minimum weight edges connecting them to
...
void initGraphAdjMatrix(int arr[][V]) {
    int i, j;
...
// Add edges
void addGraphEdge(int arr[][V], int i, int j) {
    arr[i][j] = 1;
    arr[j][i] = 1;
}
