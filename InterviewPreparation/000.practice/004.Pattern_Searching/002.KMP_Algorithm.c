...
/*
    ...
    For each sub-pattern pat[0..i] where i = 0 to m-1, lps[i] stores length of
    the maximum matching proper prefix which is also a suffix of the sub-pattern pat[0..i].
    ...
    We also know (from above definition) that lps[j-1] is count of characters of pat[0…j-1]
    that are both proper prefix and suffix.
    From above two points, we can conclude that we do not need to match these lps[j-1]
    characters with txt[i-j…i-1] because we know that these characters will anyway match.
...
void
printArray(int *arr, char* pat, int len){
    for (int i = 0; i< len; i++) {
        printf("%c ", pat[i]);
    }
    printf("\r\n");
...
void computeLPSArray(char* pat, int M, int* lps)
{
    ...
    int i = 1;
    while (i < M) {
        printArray(lps, pat, M);
        if (pat[i] == pat[len]) {
            len++;
            lps[i] = len;
            i++;
            printf("pat[i] == pat[len]: i=%d, len=%d, pat[i]=%c, pat[len]=%c\r\n", i, len, pat[i], pat[len]);
        }
        else // (pat[i] != pat[len])
        {
            ...
            if (len != 0) {
                printf("len != 0: len=%d, lps[len - 1]=%d\r\n", len, lps[len - 1]);
                len = lps[len - 1];
